...
const handleSubmit = async () => {
  ...
  if(product) newProduct.value = product;
  ...
  // TODO: limpiar los archivos creados
  filesToUpload.value = [];
...
const productSchema = z.object({
  slug: z.string().nonempty('Dato requerido'),
  name: z.string().nonempty('Dato requerido'),
  description: z.string().nonempty('Dato requerido'),
  price: z.number().min(0, 'Dato requerido'),
});
...
const handleFilesChange = ( event: Event ) => {
  ...
  // todo: crear vista previa al archivo a subir
...
const removeFilePreview = (index: number) => {
  filesToUploadPreviews.value = filesToUploadPreviews.value.filter((files, i) => i !== index);
        ...
        <dl class="mt-4 grid gap-4 sm:grid-cols-2">
          <div class="space-y-1">
            ...
            <dd class="text-gray-900 dark:text-gray-100">
              {{ longDateTimeFormat(newProduct.createdAt) }}
            </dd>
          </div>
          <div class="space-y-1">
            <dt class="text-sm font-medium text-gray-600 dark:text-gray-400">
            ...
            <dd class="text-gray-900 dark:text-gray-100">
              {{ longDateTimeFormat(newProduct?.updatedAt) }}
            </dd>
          </div>
        </dl>
      </section>
